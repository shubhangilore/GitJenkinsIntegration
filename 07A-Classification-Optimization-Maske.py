...
@filename: Classification-Optimization
@dataset: MedicalInsuranceMasked
@author: cyruslentin
"""
...
# read dataset
df  = pd.read_csv(r'C:\PY-ML\Ml test 1/glassm-data.csv')
...
print(df['Na'].unique())
df['Na'] = leCol2.fit_transform(df['Na'])
print(df['Na'].unique())
print("Done ...")
...
print(utils.OutlierValues(df))
 ...
 # handle outlier
lol, uol = utils.OutlierLimits(df['Ba'])
print("Lower Outlier Limit:",lol)
print("Upper Outlier Limit:",uol)
df['Ba'] = np.where(df['Ba'] < lol, lol, df['Ba'])
df['Ba'] = np.where(df['Ba'] > uol, uol, df['Ba'])

# handle outlier
lol, uol = utils.OutlierLimits(df['Fe'])
print("Lower Outlier Limit:",lol)
print("Upper Outlier Limit:",uol)
df['Fe'] = np.where(df['Fe'] < lol, lol, df['Fe'])
df['Fe'] = np.where(df['Fe'] > uol, uol, df['Fe'])

# handle outlier
lol, uol = utils.OutlierLimits(df['Ka'])
print("Lower Outlier Limit:",lol)
print("Upper Outlier Limit:",uol)
df['Ka'] = np.where(df['Ka'] < lol, lol, df['Ka'])
df['Ka'] = np.where(df['Ka'] > uol, uol, df['Ka'])
...
# handle zeros
df['Mg'] = np.where(df['Mg']==0,None, df['Mg'])
df['Ka'] = np.where(df['Ka']==0,None, df['Ka'])
df['Ba'] = np.where(df['Ba']==0,None, df['Ba'])
df['Fe'] = np.where(df['Fe']==0,None, df['Fe'])
